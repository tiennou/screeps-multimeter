...
module.exports = function (multimeter) {
    ...
    function getWatchExpressions(shard) {
        return multimeter.api.userMemoryGet('watch', shard).then((val) => {
            if (val) {
                if (val.data && val.data.expressions) {
                    ...
                    multimeter.log(
                        'Watch plugin disabled: Missing Memory.watch.expressions for ' +
                            shard +
                            '. Is watch-client.js installed?',
                    );
                    return null;
                }
            } else {
                multimeter.log(
                    'Watch plugin disabled: Missing Memory.watch for ' +
                        shard +
                        '. Is watch-client.js installed?',
                );
                return null;
    ...
    function setWatch(name, expression) {
        ...
        return multimeter.api.userMemorySet(
    ...
    function updateStatusBar() {
        ...
        if (shards.length > 0) {
            ...
            let text = shards
                .map(
                    (shard) =>
                        '{grey-fg}' + shard + '{/} ' + statusValues[shard],
                )
                .join('\n');
    ...
    function commandWatch(args) {
        ...
        if (args.length < 2) {
            ...
            setWatch(target, null)
                .then(() => {
                    multimeter.log(
                        target + ' watch disabled for ' + shard + '.',
                    );
                })
                .catch(errorHandler);
        } else {
