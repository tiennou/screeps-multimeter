...
class Gauges extends blessed.layout {
    ...
    update(cpu_current, cpu_limit, mem_current, mem_limit) {
        if (Number.isNaN(parseInt(cpu_current, 10))) {
            ...
            this.cpuLabel.setContent(
                printf('CPU: %3d/%3d', cpu_current, cpu_limit),
            );
...
module.exports = class Multimeter extends EventEmitter {
    ...
    async connect(serverName) {
        ...
        if (userInfo.cpuShard) {
            this.console.log(
                'Found shards: ' + JSON.stringify(userInfo.cpuShard),
            );
            ...
            if (shards.length) {
                ...
                this.shard = _.maxBy(
                    shards,
                    (shard) => userInfo.cpuShard[shard],
                );
            }
            // Filter to only shards with CPU allocated
            this.shards = _.filter(
                shards,
        ...
        this.api.socket.subscribe('console', (event) => {
            ...
            if (data.messages) {
                data.messages.log.forEach((l) =>
                    this.console.addLines('log', l, data.shard),
                );
    ...
    loadPlugins() {
        ...
        _.each(this.config.plugins, (name) => {
            const resolved = path.dirname(
                path.resolve(this.configManager.filename),
            );
    ...
    commandHelp(args) {
        if (args.length > 0) {
            ...
            if (command) {
                ...
                this.log(
                    'No help available for /' + name + ': not a valid command',
                );
            }
        } else {
            let list = _.sortBy(
